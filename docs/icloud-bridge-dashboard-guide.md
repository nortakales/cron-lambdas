...
> timestamp, arriving in a burst when the Mac's `chat.db` is next checkpointed.
> A 30s poke agent now caps this at about a minute, but do not design as though
> ingest order matches `createdAt`.
>
